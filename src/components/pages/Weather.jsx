...
const handleRealTimeWeather = async () => {
    setRealTimeLoading(true);
    try {
      const realTimeData = await weatherService.getRealTimeWeather();
      
      // Update today's weather with real-time data
      setWeather(prevWeather => {
        const updatedWeather = [...prevWeather];
        if (updatedWeather.length > 0) {
          updatedWeather[0] = realTimeData;
        } else {
          updatedWeather.push(realTimeData);
        }
      ...
      toast.success("Today's weather updated with real-time data!");
    } catch (error) {
      console.error("Failed to get real-time weather:", error);
      const { toast } = await import('react-toastify');
      toast.error(error.message || "Failed to get real-time weather");
    } finally {
  ...
  return (
    ...
    <div
        ...
        <div>
            <h1
                className="text-3xl font-bold bg-gradient-to-r from-primary-600 to-primary-800 bg-clip-text text-transparent">Weather Forecast
                          </h1>
            <p className="text-gray-600 mt-2">Plan your farm activities with accurate weather information
                          </p>
        </div>
        <div className="flex items-center space-x-4">
            <button
                onClick={handleRealTimeWeather}
                disabled={realTimeLoading}
                className="flex items-center space-x-2 px-4 py-2 bg-primary-500 hover:bg-primary-600 disabled:bg-primary-300 text-white rounded-md transition-colors font-medium">
                {realTimeLoading ? <>
                    <ApperIcon name="RefreshCw" className="h-4 w-4 animate-spin" />
                    <span>Updating...</span>
                </> : <>
                    <ApperIcon name="CloudRain" className="h-4 w-4" />
        ...
        {todayWeather && <Card className="bg-gradient-to-br from-blue-50 to-indigo-100 border-blue-200">
        ...
        {forecastWeather.length > 0 && <Card>
            ...
            <div
                ...
                {forecastWeather.map((dayWeather, index) => <WeatherCard key={index} weather={dayWeather} />)}
            </div>
        </Card>}
        ...
        <Card
            ...
            <div className="flex items-center space-x-3 mb-6">
                <div
                    className="h-12 w-12 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        {todayWeather?.condition === "sunny" && <div className="flex items-start space-x-3 p-3 bg-white rounded-lg">
                        ...
                        {todayWeather?.condition === "rainy" && <div className="flex items-start space-x-3 p-3 bg-white rounded-lg">
                        ...
                        {todayWeather?.precipitation > 70 && <div className="flex items-start space-x-3 p-3 bg-white rounded-lg">
